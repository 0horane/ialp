...
-- *-+-distrib-l {a} {b} {zero} =  (a + b) * zero ≡⟨ *-zero-right {a + b} ⟩ zero ≡⟨ refl ⟩ zero + zero  ≡⟨ cong (λ x -> x + zero) (sym (*-zero-right {a}) ) ⟩ (a * zero) + zero ≡⟨ {!!} ⟩ {!!} ∎
-- *-+-distrib-l {a} {b} {suc c} =  begin (a + b) * (suc c) ≡⟨ {!!} ⟩ {!!} ∎
*-+-distrib-l = {!!}

-- *-+-distrib-l {zero} {b} {c} =  (zero + b) * c ≡⟨ {!!} ⟩ {!!} ∎ 
...
-- primero necesitamos constructor de ≤
≤-refl : {n : ℕ} -> n ≤ n
≤-refl = zero , refl
